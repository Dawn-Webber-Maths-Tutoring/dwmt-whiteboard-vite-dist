import{o as x,p as E,b as I,m as R,e as H,i as N,k as U,j as D,l as j,Y as O,Z as G}from"./UserMenu-B3lybQXW.js";import{c as J}from"./studentPicker--1eQP9lv.js";import{g as V,s as z}from"./authSession-DVyUGB65.js";const W=()=>{try{const t=localStorage.getItem("dwmt-selected-student");if(!t)return[];const e=JSON.parse(t);if(Array.isArray(e))return e.filter(n=>n&&n.username&&n.email)}catch(t){console.warn("Failed to load selected students for activity filter",t)}return[]},Y=t=>{try{if(t.length>0){localStorage.setItem("dwmt-selected-student",JSON.stringify(t));return}localStorage.removeItem("dwmt-selected-student")}catch(e){console.warn("Failed to save selected students for activity filter",e)}},F=(t,e,n)=>{const i=t.querySelector("#studentAvatarGroup");if(!i)return;if(e!=="tutor"||n.length===0){i.hidden=!0,i.innerHTML="";return}const o=document.createDocumentFragment();n.forEach(s=>{const r=document.createElement("button");r.type="button",r.className="student-avatar";const d=s.avatarUrl||"/default_avatar.png";r.innerHTML=`<img src="${d}" alt="" aria-hidden="true" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;" />`;const l=s.displayName?.trim()||s.email||s.username;r.title=l,r.setAttribute("aria-label",`Selected student: ${l}`),o.appendChild(r)}),i.hidden=!1,i.innerHTML="",i.appendChild(o)},h=t=>{if(!t)return"—";const e=new Date(t);return Number.isNaN(e.getTime())?t:e.toLocaleString()},Z=t=>{const e=t.split("/").pop()||t;return decodeURIComponent(e)},_=()=>`
