import{p as fe,o as ve,b as he,m as ge,e as ye,i as be,k as Ce,j as Se,l as $e,x as xe,Z as De,a4 as we,a0 as Le}from"./UserMenu-D2SDV5jZ.js";import{g as qe,s as Ee}from"./authSession-DDHs5wue.js";const V=[{value:"open",label:"Open"},{value:"in-progress",label:"In progress"},{value:"require-clarification",label:"Needs clarification"},{value:"testing",label:"Testing"},{value:"tested",label:"Tested"},{value:"resolved",label:"Resolved"},{value:"duplicate",label:"Duplicate"},{value:"wont-fix",label:"Won't fix"}],Ae=()=>`
...
${m}`.trim()})},Ue=e=>{const n=G(e.context);return(Array.isArray(n.recentUiEvents)?n.recentUiEvents:[]).map(i=>{const p=typeof i.type=="string"?i.type:"event",m=typeof i.target=="string"?i.target:"unknown",d=typeof i.detail=="string"&&i.detail.trim()?` · ${i.detail.trim()}`:"";return`${z(typeof i.ts=="string"?i.ts:void 0)} · ${p} · ${m}${d}`})},ke=e=>{const n=e.kind==="video",o=e.filename||"attachment";return`
  ...
  `},Me=async()=>{const e=document.querySelector("#app");if(!e)throw new Error("Missing #app root");const n=fe().map(s=>s.trim().toLowerCase()),o=n.includes("admin");if(!o){window.location.href="/whiteboard";return}const i=ve(),p=n.includes("mailbox");e.innerHTML=Ae(),e.appendChild(he()),ge(),ye();const m=e.querySelector("#issuesAppBarContainer"),d=e.querySelector("#issuesEnvironmentLabel"),f=e.querySelector("#issuesRefreshBtn"),c=e.querySelector("#issuesListStatus"),P=e.querySelector("#issuesListSummary"),x=e.querySelector("#issuesList"),q=e.querySelector("#issuesSearchInput"),E=e.querySelector("#issuesStateFilter"),A=e.querySelector("#issuesStatusFilter"),I=e.querySelector("#issuesDetailEmpty"),R=e.querySelector("#issuesDetailCard"),_=e.querySelector("#issuesDetailMeta"),W=e.querySelector("#issuesDetailTitle"),T=e.querySelector("#issuesDetailStatusBadge"),J=e.querySelector("#issuesDetailReporter"),Z=e.querySelector("#issuesDetailDescription"),K=e.querySelector("#issuesDetailClarification"),X=e.querySelector("#issuesDetailClarificationQuestion"),F=e.querySelector("#issuesDetailClarificationResponse"),Y=e.querySelector("#issuesDetailAttachmentCount"),ee=e.querySelector("#issuesDetailAttachments"),se=e.querySelector("#issuesDetailContext"),te=e.querySelector("#issuesDetailHttpFailureCount"),ie=e.querySelector("#issuesDetailHttpFailures"),ae=e.querySelector("#issuesDetailConsoleLogCount"),ne=e.querySelector("#issuesDetailConsoleLogs"),oe=e.querySelector("#issuesDetailUiEventCount"),re=e.querySelector("#issuesDetailUiEvents"),B=e.querySelector("#issuesDetailStatusSelect"),U=e.querySelector("#issuesDetailStatusNote"),D=e.querySelector("#issuesSaveBtn");if(!m||!d||!f||!c||!P||!x||!q||!E||!A||!I||!R||!_||!W||!T||!J||!Z||!K||!X||!F||!Y||!ee||!se||!te||!ie||!ae||!ne||!oe||!re||!B||!U||!D)throw new Error("Issues page failed to mount");d.textContent=Re();let le=0;if(p)try{le=await be()}catch(s){console.warn("Failed to load unread inbox count for issues page",s)}const me=Ce({currentApp:"issues",onAppChange:s=>{if(s==="whiteboard"){window.location.href="/whiteboard/";return}if(s==="activity"){window.location.href="/activity/";return}if(s==="inbox"){window.location.href="/mailbox/";return}if(s==="worksheet-builder"){window.location.href="/worksheet-builder/";return}if(s==="notes"){window.location.href="/notes/";return}if(s==="user-management"){window.location.href="/user-management/";return}s==="profile"&&(window.location.href="/profile/")},userRole:i,isAdmin:o,hasMailboxAccess:p,unreadInboxCount:le,rightContent:Se()});m.appendChild(me),await $e({root:e,profileHref:"/profile/",signOutHref:"/auth",hideSelectStudent:!0,labelMode:"display-name-or-email",onSignOutClick:()=>{Ee(),window.location.href="/auth"}});let C=[],r="",a=null;const v={query:"",state:"open",status:""},k=()=>{const s=Ie(v.query);return C.filter(t=>v.state==="open"&&de(t.status)||v.state==="closed"&&!de(t.status)||v.status&&t.status!==v.status?!1:s?[t.summary,t.description,t.reporterEmail||"",t.humanReference||""].join(" ").toLowerCase().includes(s):!0)},b=()=>{const s=k();if(P.textContent=`${s.length} issue${s.length===1?"":"s"} shown`,s.length===0){x.innerHTML='<div class="issues-empty-list">No issues match the current filters.</div>';return}x.innerHTML=s.map(t=>{const u=t.bugId===r?" is-active":"",h=t.reporterEmail?`<span class="issues-list-item-reporter">${l(t.reporterEmail)}</span>`:"";return`
        ...
        `}).join("")},S=()=>{if(!a){R.hidden=!0,I.hidden=!1;return}I.hidden=!0,R.hidden=!1,_.textContent=`${a.humanReference||a.bugId} · Created ${$(a.createdAt)} · Updated ${$(a.updatedAt)}`,W.textContent=a.summary?.trim()||"Issue",T.textContent=pe(a.status),T.dataset.status=a.status,J.textContent=`${a.reporterDisplayName} · ${a.reporterEmail}`,Z.textContent=a.description||"No description.";const s=typeof a.clarificationQuestion=="string"?a.clarificationQuestion.trim():"",t=typeof a.statusComment=="string"?a.statusComment.trim():"",u=s||(a.status==="require-clarification"?t||"Clarification requested.":"");K.hidden=!u,X.textContent=u;const h=typeof a.clarificationResponse=="string"&&a.clarificationResponse.trim()?a.clarificationResponse.trim():"";F.hidden=!h,F.textContent=h;const w=Array.isArray(a.attachments)?a.attachments:[];Y.textContent=`${w.length} file${w.length===1?"":"s"}`,ee.innerHTML=w.length>0?w.map(L=>ke(L)).join(""):'<div class="issues-empty-attachments">No attachments for this issue.</div>',se.innerHTML=Te(a).map(L=>`
          <div class="issues-context-item">
            <span>${l(L.label)}</span>
            <strong>${l(L.value)}</strong>
          </div>
        `).join("");const M=Fe(a);te.textContent=`${M.length} item${M.length===1?"":"s"}`,ie.innerHTML=Q(M,"No failed requests captured for this issue.");const O=Be(a);ae.textContent=`${O.length} item${O.length===1?"":"s"}`,ne.innerHTML=Q(O,"No console logs captured for this issue.");const j=Ue(a);oe.textContent=`${j.length} item${j.length===1?"":"s"}`,re.innerHTML=Q(j,"No recent UI events captured for this issue."),B.value=a.status,U.value=typeof a.statusComment=="string"?a.statusComment:""},H=async s=>{r=s,a=null,b(),S(),g(c,"Loading issue details…","info");try{a=(await De(s)).bug,g(c,"","success")}catch(t){r="";const u=t instanceof Error?t.message:"Failed to load issue details";g(c,u,"error")}b(),S()},N=async()=>{const s=k();if(s.some(u=>u.bugId===r)){b();return}if(r=s[0]?.bugId||"",!r){a=null,b(),S();return}await H(r)},ce=async()=>{f.disabled=!0,g(c,"Loading issues…","info");try{C=(await we()).bugs||[],g(c,"","success"),(!r||!C.some(t=>t.bugId===r))&&(r=k()[0]?.bugId||""),b(),r?await H(r):(a=null,S())}catch(s){C=[],r="",a=null,b(),S();const t=s instanceof Error?s.message:"Failed to load issues";g(c,t,"error")}finally{f.disabled=!1}};x.addEventListener("click",s=>{const h=s.target?.closest("[data-bug-id]")?.dataset.bugId;!h||h===r||H(h)}),q.addEventListener("input",()=>{v.query=q.value,N()}),E.addEventListener("change",()=>{v.state=E.value,N()}),A.addEventListener("change",()=>{v.status=A.value||"",N()}),f.addEventListener("click",()=>{ce()}),D.addEventListener("click",async()=>{if(r){D.disabled=!0;try{const s=await Le(r,B.value,U.value.trim());a=s.bug,C=C.map(t=>t.bugId===r?{...t,status:s.bug.status,statusComment:typeof s.bug.statusComment=="string"?s.bug.statusComment:t.statusComment,updatedAt:typeof s.bug.updatedAt=="string"?s.bug.updatedAt:t.updatedAt,clarificationQuestion:typeof s.bug.clarificationQuestion=="string"?s.bug.clarificationQuestion:t.clarificationQuestion,clarificationResponse:typeof s.bug.clarificationResponse=="string"?s.bug.clarificationResponse:t.clarificationResponse}:t),b(),S(),g(c,"Issue updated.","success")}catch(s){const t=s instanceof Error?s.message:"Failed to update issue";g(c,t,"error")}finally{D.disabled=!1}}});const ue=await qe().catch(()=>null);if(!ue||!ue.isValid()){window.location.href="/auth";return}await ce()};export{Me as mountIssuesPage};
